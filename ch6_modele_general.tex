...
\caption{\fix{quelque chose de mieux ?}Illustration de notre modèle canonique JOLIMAS. Pour chaque poses de caméra réelles ($\Pi_1$, $\Pi_2$ et $\Pi_3$), nous créons des poses de caméra virtuelles ($\Pi_{s, 1}$, $\Pi_{s, 2}$ et $\Pi_{s, 3}$) calculées à partir des points d'intensité maximale de chaque spécularités. En synthétisant la forme des spécularités sur le plan tangent aux points d'intensité maximale, nous sommes capable de reconstruire une quadrique localisée près de la source de lumière $\mbf{L}$ invariant à la courbure de la surface.}
\label{fig:canonical}
\end{figure}
...
\subfigure[]
{
    \includegraphics[width=\linewidth, height=8cm]{homography.pdf}
}
...
Dans cette section, nous présentons deux résultats de prédiction de spécularité sur des séquences réelles une réplique de fusée en métal et un livre à couverture brillante.
...
\subfigure[]
{
    \includegraphics[width=0.3\linewidth]{normal}
}
\subfigure[]
{
    \includegraphics[width=0.3\linewidth]{normal_map}
}
\caption{Calcul de la carte de normale. A partir du nuage de point de la carte de profondeur (a), nous utilisons la bibliothèque PCL pour calculer les normales à partir des points 3D voisins (b). La carte de normale obtenue est suffisamment précise pour notre application (c).}
