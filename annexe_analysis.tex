...
La fonction d'intensité du modèle de Blinn-Phong d'un point 3D d'une surface $\mathbf{P}$ est donné par :
\begin{equation}
 \begin{aligned}
...
\end{equation}
D'une façon similaire au modèle de Phong, nous voulons analyser les isocontours d'une spécularité à partir de l'équation \eqref{eq:blinn_phong_dev}.
...
\label{sec:tau_one}
L'étude du cas  $\tau = 1$ inclue les points aillant une intensité maximale sur la surface $S$. Pour le modèle de Phong, cette étude est intéressant car pour $\tau = 1$, les termes de degré trois et quatre disparaissent. Les termes restant sont :
\begin{flalign*}
...
\end{equation*}
Nous avons vu que le deuxième et le troisième terme (les coefficients associé au degré 0) sont aussi égaux à $\mb{V}^\top [\mb{R}]_\times^2\mb{V}$, ce qui conclue.

\section{Point d'intensité maximale dans une spécularité sous le modèle de Phong}
Nous souhaitons trouver le point d'intensité maximale d'une spécularité créé par une source de lumière sur une surface plane dont l'intensité est donnée par le modèle de Phong. Sans perte de généralité, nous travaillons en coordonnées monde pour la surface plane de la scène $S \subset \mathbb{R}^3$ soit le plan  $(\vect{XY})$. En détails, $S = \{\vect{P}\text{ } | \text{ } P_Z = 0\}$. Nous pouvons ainsi paramétrer $S$ par un point $\vect{p} \in \mathbb{R}^2$ et définir $\vect{P} = \mathop{\text{stk}}(\vect{p}, 0)$. Au point $\vect{p}$ la composante spéculaire $I_s(\vect{p})$ est donnée par :
\begin{equation}
...
\end{equation}
Nous voulons étudier l'existence, la localisation et l'unicité de $\hat{\vect{p}}$. A proprement parlé, nous réalisons une étude du point d'intensité maximale de terme spéculaire de Phong. En pratique, ce terme domine les termes diffus et ambiant, et  $\hat{\vect{p}}$ sera ainsi une bonne approximation du point d'intensité maximale dans les cas incluant la lumière ambiante et la réflexion diffuse.
...
\begin{theorem}
Pour un point de vue et une source de lumière donnés aux positions  $\vect{V}, \vect{L} \in \mathbb{R}^3$ respectivement, localisés sur le même côté du plan $S$ coïncidant avec le plan $\vect{XY}$ du plan de référence des coordonnées monde, la spécularité créée par le terme spéculaire de Phong a un point d'intensité maximale localisée à $\frac{1}{V_Z + L_Z}\begin{bmatrix}
...
\end{flalign*}
Cette équation peut être reformulée en une forme plus compacte en utilisant des projecteurs tel que :
\begin{equation}
...
\end{flalign*}
où $\vect{A}$, $\vect{B} \in \mathbb{R}^3$, $[\vect{A}]_\times \in \mathbb{R}^{3 \times 3}$ une matrice antisymétrique tel que  $[\vect{A}]_\times \vect{B} = \vect{A} \times \vect{B}$. Puisque $[\vect{A} \times \vect{B}]_\times = -[\vect{B} \times \vect{A}]_\times$ nous avons :
\begin{equation}
